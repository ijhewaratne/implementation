...
if not buildings_gdf.empty and street_gdf is not None:
    ...
    building_ids = buildings_proj.get(
        "GebaeudeID",
    ...
    pos = {
        ...
        for building_id, centroid in zip(building_ids, building_centroids)
    }
    ...
    print(
        f"Created network graph with {mst_graph.number_of_nodes()} nodes and {mst_graph.number_of_edges()} edges."
    )
    ...
    for _, building in buildings_proj.iterrows():
        ...
        building_data = {
            ...
            "properties": {"area": building.geometry.area, "perimeter": building.geometry.length},
        }
    ...
    edge_artists = nx.draw_networkx_edges(
        mst_graph, pos, ax=ax, edge_color="#0066ff", width=2, alpha=0.7
    )

    # 5. Plot the MST graph nodes
    node_artists = nx.draw_networkx_nodes(
        mst_graph, pos, ax=ax, node_size=60, node_color="blue", edgecolors="black"
    )
