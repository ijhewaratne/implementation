...
def test_velocity_limit_enforcement(tmp_path):
    ...
    segs = [Segment("S1", 100.0, 0.015, 0, "P1", is_supply=True)]  # High flow: 0.015 m³/s
    ...
    # Should be within velocity limit (or at least better than smaller DN)
    assert m2["v_max"] < m["v_max"]  # Larger DN should have lower velocity
...
def test_velocity_target_respect(tmp_path):
    ...
    # Low flow rate that should be feasible
    segs = [Segment("S1", 150.0, 0.005, 0, "P1", is_supply=True)]  # 0.005 m³/s
    ...
    # Test manual assignment instead of initial_feasible
    m = opt.evaluate_quick({"S1": 80})  # Use DN 80
...
def test_velocity_edge_cases(tmp_path):
    ...
    # Very high flow rate
    segs = [Segment("S1", 50.0, 0.025, 0, "P1", is_supply=True)]  # 0.025 m³/s
    ...
    # Test with largest DN - should have lower velocity than smallest DN
    m_large = opt.evaluate_quick({"S1": 100})
    assert m_large["v_max"] < m_small["v_max"]  # Larger DN should have lower velocity
