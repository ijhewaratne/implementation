...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="integrate_npv_with_dh_system", description="NPV–DH diameter optimization & reporting"
    )
    p.add_argument(
        "--optimize-dn",
        action="store_true",
    ...
    p.add_argument(
        "--segments-csv",
        type=Path,
        help="Segments CSV (seg_id,length_m,V_dot_m3s,Q_seg_W,path_id,is_supply[,DN])",
    )
    p.add_argument("--street", type=str, default="Unnamed Street", help="Street label for reports")
    p.add_argument(
        "--out-dir", type=Path, default=Path("results_test/npv_dh"), help="Output directory"
    )
    p.add_argument(
...
def main(argv=None) -> int:
    ...
    design = dict(
        ...
        hours=args.hours,
        v_feasible_target=args.v_target,
    ...
    if args.monte_carlo and args.monte_carlo > 0:
        try:
            ...
            mc = run_monte_carlo(
                assignment, per_seg_df, design, econ, catalog_df, n=int(args.monte_carlo)
            )
