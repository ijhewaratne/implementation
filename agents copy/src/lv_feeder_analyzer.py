...
def map_buildings_to_feeders(
    buildings_df: pd.DataFrame, feeders_df: pd.DataFrame
) -> Dict[str, str]:
    if (
        "building_id" in buildings_df.columns
        and "building_id" in feeders_df.columns
        and "feeder_id" in feeders_df.columns
    ):
        m = dict(
            pd.merge(
            ...
            .fillna({"feeder_id": "F0"})
            .set_index("building_id")["feeder_id"]
        )
...
def _run_hour_for_feeder_pp(net, lv_bus: int, p_kw: float, q_kvar: float = 0.0) -> Dict[str, float]:
    ...
    if idx is None:
        idx = pp.create_sgen(
            net, bus=lv_bus, p_mw=p_mw, q_mvar=q_mvar, name=f"agg_load_bus{lv_bus}"
        )
                ...
                rows.append(
                    dict(
                        ...
                        **{
                            ...
                            "violates_voltage_outside_±10%": (
                                vmin < v_limits[0] or vmax > v_limits[1]
                            ),
                        },
                    )
                )
        ...
        for fid, p_kw in feeder_kw.items():
            ...
            rows.append(
                dict(
                    feeder_id=fid,
                    hour=int(h),
                    utilization_max=kpis["utilization_max"],
                    voltage_min=kpis["voltage_min"],
                    voltage_max=kpis["voltage_max"],
                    **{
                        "violates_util>=0.8": kpis["utilization_max"] >= util_threshold,
                        "violates_voltage_outside_±10%": (
                            kpis["voltage_min"] < v_limits[0] or kpis["voltage_max"] > v_limits[1]
                        ),
                    },
                )
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--backend", default="heuristic", choices=["pandapower", "heuristic"])
