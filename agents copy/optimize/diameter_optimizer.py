...
class DiameterOptimizer:
    ...
    def evaluate_quick(self, assignment: Dict[str, int]) -> Dict:
        ...
        try:
            # Initialize results
            per_segment = {}
            # path_stats will be built later from supply segments only
            all_velocities = []
            ...
            # --- Build path-level stats from SUPPLY segments only ---
            from collections import defaultdict

            path_dp = defaultdict(float)
            path_Vdot_peak = defaultdict(float)

            for seg in self.segments:
                # Only supply segments contribute to worst-path head (pump sits on supply side)
                if not seg.is_supply:
                    continue

                seg_metrics = per_segment.get(seg.seg_id)
                if seg_metrics is None:
                    continue

                # Sum pressure drops along each supply path
                # If you have minor-loss terms, include them here (seg_metrics.get('dp_minor', 0.0))
                path_dp[seg.path_id] += float(seg_metrics["dp"])

                # Track representative/max flow for the path (used for pump energy)
                # If your semantics want sum, change to +=; most designs use max/representative
                path_Vdot_peak[seg.path_id] = max(path_Vdot_peak[seg.path_id], float(seg.V_dot_m3s))

            # Materialize dict for reportability
            path_stats = {
                pid: {
                    "dp_Pa": dp,
                    "V_dot_peak_m3s": path_Vdot_peak[pid],
                }
                for pid, dp in path_dp.items()
            }

            # Worst path among SUPPLY paths only
            dp_path_max_Pa = max(path_dp.values(), default=0.0)
            worst_pid = max(path_dp, key=path_dp.get) if path_dp else None
            Vdot_worst_m3s = path_Vdot_peak.get(worst_pid, 0.0)

            # Head requirement (m), pump energy (MWh) based on worst supply path only
            rho = float(self.design["rho"])
            g = 9.81
            head_required_m = dp_path_max_Pa / (rho * g) if dp_path_max_Pa > 0 else 0.0

            pump_MWh = annual_pump_energy_mwhel(
                dp_sum_pa=dp_path_max_Pa,
                V_dot_path_m3s=Vdot_worst_m3s,
                eta_pump=float(self.design["eta_pump"]),
                hours=float(self.design["hours"]),
            )
            ...
            return {
                "v_max": v_max,
                "dp_path_max_Pa": dp_path_max_Pa,
                "head_required_m": head_required_m,
                "pump_MWh": pump_MWh,
