...
def annual_pump_energy_mwhel(
    dp_sum_pa: float, V_dot_path_m3s: float, eta_pump: float, hours: float
) -> float:
    """
...
def npv(
    capex: float, annual_cost: Union[float, List[float], Tuple[float, ...]], years: int, r: float
) -> float:
    """
    ...
    if isinstance(annual_cost, (int, float)):
        ...
        if r == 0:
            ...
            pv_costs = sum(
                cost / ((1 + r) ** (i + 1)) for i, cost in enumerate(annual_cost[:years])
            )
