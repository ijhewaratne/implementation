...
def build_validation_suite(
    out_path: str, lfa: dict = None, en13941: dict = None, feeder_df=None, econ_bench: dict = None
) -> str:
    """
    ...
    def badge(ok):
        return (
            "<span style='background:%s;color:#fff;padding:2px 8px;border-radius:12px'>%s</span>"
    ...
    if en13941:
        lines.append(
            f"<h3>EN 13941</h3> velocity {badge(bool(en13941.get('velocity_ok', False)))} ΔT {badge(bool(en13941.get('deltaT_ok', False)))}"
        )

    if feeder_df is not None:
        lines.append(
            "<h3>VDE/FNN</h3> "
            + ("no violations" if not feeder_df.any().any() else "violations present")
        )
