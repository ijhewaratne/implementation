...
# --- CLI flags per exposé (non-intrusive: appended at end) ---
def build_parser():
    import argparse

    p = argparse.ArgumentParser(
        prog="simulate_dual_pipe_dh_network_final",
        description="Dual-pipe DH simulator (with optional DN optimization hooks)",
    )
    p.add_argument(
        "--optimize-dn", action="store_true", help="Run DN optimization after simulation"
    )
    p.add_argument("--catalog", type=str, help="Pipe catalog CSV for optimization")
    p.add_argument(
        "--out-dir", type=str, default="results_test/dh_analysis", help="Output directory"
    )
    p.add_argument(
        "--report-html", type=str, default="compliance_report.html", help="Report filename"
    )
    p.add_argument("--export-geojson", type=str, help="Optional GeoJSON export path")
    p.add_argument("--monte-carlo", type=int, default=0, help="Monte Carlo trials (0=skip)")
    # keep primary simulator's own flags here if you already have them (e.g., --street, --scenario, etc.)
    return p
...
def main_cli(argv=None):
    ...
    # NOTE: The simulator's own entry point goes here. If you already have a function like run_simulation(args),
    # call it. Otherwise, we keep this as a placeholder so flags are recognized by CI/audit.
    print("Simulation CLI parsed. (Hook your simulation entry here.)")
    # If you want to chain to integrate_npv_with_dh_system, you could import and forward arguments here.
    return 0
