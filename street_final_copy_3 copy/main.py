...
def run_pipeline(config_file):
    ...
    if config.get("run_data_preparation", True):
        ...
        # Use the new CRS-aware data preparation function
        selected_buildings = (
            config.get("selected_buildings", None) if config.get("test_mode", False) else None
        )
        bldg_gdf, edges, nodes, G = data_preparation.load_and_prepare_data(
            buildings_file=config["buildings_file"],
            osm_file=config["osm_file"],
            output_dir=output_dir,
            selected_buildings=selected_buildings,
            ensure_projected=True,  # Always ensure projected CRS for accurate calculations
        )
