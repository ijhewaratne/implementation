...
def test_flags_trigger_with_pandapower():
    ...
    hourly_kw = pd.DataFrame({
        ...
        "1": [250.0, 300.0, 250.0] # 800 kW total -> near 0.8 pu for 1 MVA equiv, but our trafo is 0.4 MVA so loading will exceed 80%
    }, index=["B1","B2","B3"])
    ...
    df = run_feeder_studies(feeders_model, b2f, hourly_kw, hours, backend="pandapower",
                            util_threshold=0.8, v_limits=(0.90, 1.10), power_factor=0.98)
...
def test_pandapower_realistic_values():
    ...
    # Boolean flags (check they are boolean-like)
