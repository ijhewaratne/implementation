...
def test_write_compliance_report_smoke(tmp_path):
    ...
    assert "NPV" in html_content
    assert "Pump MWh" in html_content
    assert "ΔT design" in html_content
    ...
    assert "1.55 m/s" in html_content   # Max velocity
...
def test_write_compliance_report_validation():
    ...
    summary = {
        'npv_eur': 100000, 'capex_eur': 50000, 'opex_eur_per_a': 5000,
        'pump_MWh': 1.0, 'heat_loss_MWh': 10.0, 'v_max': 1.2,
        'head_required_m': 10.0, 'deltaT_design_k': 30.0,
        'velocity_ok': True, 'deltaT_ok': True
    }
    ...
    valid_df = pd.DataFrame({
        'seg_id': ['S1'], 'length_m': [100], 'DN': [80], 'V_dot_m3s': [0.01],
        'v_mps': [1.0], 'dp_Pa': [1000], 'h_m': [0.1], 'heat_loss_W': [100],
        'path_id': ['P1'], 'is_supply': [True]
    })
...
def test_write_compliance_report_large_table(tmp_path):
    ...
    summary = {
        'npv_eur': 100000, 'capex_eur': 50000, 'opex_eur_per_a': 5000,
        'pump_MWh': 1.0, 'heat_loss_MWh': 10.0, 'v_max': 1.0,
        'head_required_m': 10.0, 'deltaT_design_k': 30.0,
        'velocity_ok': True, 'deltaT_ok': True
    }
...
def test_export_geojson_validation():
    ...
    if not has_geopandas:
        ...
        with pytest.raises(ValueError, match="missing required column: seg_id"):
