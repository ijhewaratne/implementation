...
def main():
    ...
    ap.add_argument("--matrix", default="configs/expose_compliance.yml",
                    help="Path to compliance matrix YAML file")
    ap.add_argument("--repo", default=".",
                    help="Repository root directory")
    ap.add_argument("--out-json", default="results_test/audit/expose_audit.json",
                    help="Output JSON report path")
    ap.add_argument("--fail-on-missing", action="store_true",
                    help="Exit nonzero if any must_have item fails")
    ap.add_argument("--verbose", "-v", action="store_true",
                    help="Verbose output with evidence details")
    args = ap.parse_args()
    ...
    report["summary"]["overall"] = {
        ...
        "percentage": round((passed_requirements / total_requirements * 100) if total_requirements > 0 else 0, 1)
    }
    ...
    print(f"📊 Overall: {passed_requirements}/{total_requirements} requirements passed ({report['summary']['overall']['percentage']}%)")
    print()
