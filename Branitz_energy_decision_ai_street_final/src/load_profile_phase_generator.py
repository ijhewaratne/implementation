...
class ParallelLoadProfileGenerator:
    ...
    def is_valid_building(self, building_id: str, building_data: Dict) -> bool:
        ...
        if building_code in self.EXCLUDED_BUILDING_CODES:
            print(
                f"Überspringe Gebäude {building_id} (Code {building_code}): Ausgeschlossener Gebäudetyp"
            )
            return False
    ...
    def process_building(
        self, building_tuple: Tuple[str, Optional[Dict]]
        ...
        try:
            ...
            profile_df = generator.generate_load_profile(building_id, "2024-01-01", "2024-12-31")
            ...
            for part in building_data.get("Gebaeudeteile", []):
                ...
                if (
                    part_id and part_id != building_id
                ):  # Nur hinzufügen wenn nicht gleich building_id
            ...
            for season in TimeDefinitions.SEASONS:
                for day_type, de_day in TimeDefinitions.DAY_TYPES.items():
                    season_mask = profile_df["timestamp"].apply(lambda x: x.month in season.months)
                    ...
                    for phase in TimeDefinitions.PHASES:
                        ...
                        result[key] = round(df_filtered.loc[phase_mask, "power_kw"].mean(), 4)
    ...
    def generate_all_profiles(self, output_file: str) -> Dict:
        ...
        with mp.Pool(processes=num_processes) as pool:
            results = list(
                tqdm(
                    pool.imap_unordered(self.process_building, valid_buildings),
                    total=len(valid_buildings),
                    desc="Verarbeite Gebäude",
                )
            )

        all_results = {building_id: result for building_id, result in results if result is not None}
