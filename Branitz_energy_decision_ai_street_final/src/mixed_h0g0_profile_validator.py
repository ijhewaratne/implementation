...
class MixedH0G0ProfileValidator:
    ...
    def validate_load_ranges(self, profile_df, expected_yearly_kwh):
        ...
        return (
            ...
            and scaled_ranges["peak_load"][0] <= peak_load <= scaled_ranges["peak_load"][1]
            and scaled_ranges["day_load"][0] <= day_load <= scaled_ranges["day_load"][1]
        )
    ...
    def validate_daily_pattern(self, profile_df, expected_yearly_kwh):
        ...
        actual_values = {
            "morning": workday_data[workday_data["timestamp"].apply(lambda x: 7 <= x.hour <= 9)][
                "power_kw"
            ].mean(),
            "business": workday_data[workday_data["timestamp"].apply(lambda x: 9 <= x.hour <= 17)][
                "power_kw"
            ].mean(),
            "evening": workday_data[workday_data["timestamp"].apply(lambda x: 17 <= x.hour <= 21)][
                "power_kw"
            ].mean(),
        ...
        return all(
            abs(actual_values[period] - ((reference_values[period] * scaling_factor) / 1000))
