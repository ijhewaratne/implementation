...
class G5ProfileValidator:
    ...
    def validate_load_ranges(self, profile_df, expected_yearly_kwh):
        ...
        # Hauptgeschäftszeit
        day_mask = profile_df["timestamp"].apply(lambda x: 6 <= x.hour <= 18 and x.weekday() < 6)
    ...
    def validate_daily_pattern(self, profile_df, expected_yearly_kwh):
        ...
        actual_values = {
            "early_morning": workday_data[
                workday_data["timestamp"].apply(lambda x: 4 <= x.hour <= 7)
            ]["power_kw"].mean(),
            "morning": workday_data[workday_data["timestamp"].apply(lambda x: 8 <= x.hour <= 11)][
                "power_kw"
            ].mean(),
            "afternoon": workday_data[
                workday_data["timestamp"].apply(lambda x: 14 <= x.hour <= 17)
            ]["power_kw"].mean(),
            "night": workday_data[workday_data["timestamp"].apply(lambda x: 1 <= x.hour <= 3)][
                "power_kw"
            ].mean(),
        }
