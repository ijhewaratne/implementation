#!/usr/bin/env python3
"""
Interactive Dual-Pipe District Heating Network Runner (FIXED)

This script allows users to:
1. Select specific streets interactively
2. Create complete dual-pipe district heating networks using the selected street's buildings
...
class StreetSpecificDualPipeNetwork(ImprovedDualPipeDHNetwork):
    ...
    def load_data(self):
        ...
        if self.buildings_file and os.path.exists(self.buildings_file):
            ...
            print(f"✅ Loaded {len(self.buildings_gdf)} buildings from selected street")
        else:
...
def create_dual_pipe_network_for_street(
    ...
    try:
        ...
        network_creator = StreetSpecificDualPipeNetwork(
            results_dir=output_dir, buildings_file=buildings_file
        )
...
def main():
    ...
    print("🏗️ INTERACTIVE DUAL-PIPE DISTRICT HEATING NETWORK RUNNER (FIXED)")
    print("=" * 80)
