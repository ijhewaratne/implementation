...
from src.network_construction import build_network, validate_network, plot_network_graph,build_network_from_edges_nodes, count_edges_with_positions
...
def run_pipeline(config_file):
    ...
    if config.get("run_demand_calculation", True):
        ...
        #import geopandas as gpd
        bldg_gdf = gpd.read_file(config['bldg_env_file'])
        bldg_gdf = filter_buildings_for_test_mode(bldg_gdf, config)
    ...
    with open(config["scenario_config_file"]) as f:
        scenario_config = yaml.safe_load(f)
    scenarios = scenario_config["scenarios"]
    built_network_files = set()
    for sc in scenarios:
        scenario_type = sc["type"]
        graphml_file = sc["network_file"]
        
    

        if not os.path.exists(graphml_file):
            G = build_network_from_edges_nodes(
                bldg_gdf, edges, nodes,
                scenario_type=scenario_type,
                output_path=graphml_file,
                plot_network=False
            )
            count, edges_with_position = count_edges_with_positions(G)
            print(f"Edges with both endpoints having position: {count} / {G.number_of_edges()}")
        else:
            print(f"Network already exists: {graphml_file}")
